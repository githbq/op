define( function( require, exports, module ) {
    ...
    var SalonDataTable = MClass( M.Center ).include( {
        tplSalon: _.template( tpl.filter( '#trSalon' ).html() ),
        PAGESIZE: 20,
        elements: {
            '#dsName': 'name',
            '#dsMobile': 'mobile',
        ...
        init: function() {
            SalonDataTable.__super__.init.apply( this, arguments );
            var me = this;
            console.log( me.$view.find('.list-pager') );
        ...
        search: function() {
            this.pagination.attr['pageNumber'] = 1;
            this.load();
        },
        export: function() {
            var me = this;
            ...
            var url = location.protocol + '//' + location.host + IBSS.API_PATH + '/data/salon/export?' + queryString;
            window.open( url );
        },
        load: function() {
            var me = this;
            ...
            util.api({
                ...
                success: function( data ) {
                    if ( data.success ) {
                        ...
                        if ( content.length > 0 ) {
                            me.$tbody.html( me.tplSalon( {'content':me.collection.all() } ) );
                        } else {
    ...
    exports.init = function() {
        var $el = exports.$el;
        var salonDataTable = new SalonDataTable( { 'view': $el.find( '.m-data-salon' ) } );
    }
} );